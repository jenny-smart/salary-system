...
原理：
    直接由 Python 取得 Google OAuth token，
    呼叫 Google Sheets export API 將「薪資單」工作表輸出為 PDF。
    優先上傳至 Drive：{地區根目錄}/{期別}/{期別} 子資料夾；
    若 Drive 上傳失敗，仍回傳 PDF bytes 供 Streamlit 下載。
...
def run_pdf(
    ...
    **kwargs,
    ...
    直接使用 Python 產出 PDF。
    優先使用使用者 OAuth Drive 權限上傳；若 Drive 不可用，
    則把 PDF bytes 回傳給 salaryapp.py 提供下載按鈕。
