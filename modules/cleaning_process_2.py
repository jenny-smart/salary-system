...
def _wait_and_convert(
    ...
    if data:
        ws.update(f"A2:AC{last}", data, value_input_option="RAW")
...
def _run_common_process(ws: gspread.Worksheet, log: List[str]) -> None:
    """
    共通 QRS→U-Y→AA-AC 流程。
    Q=col17, R=col18, S=col19 → 篩選 R≠0 → 寫入 V/W/X（col22/23/24）
    U=col21（V欄姓名出現次數）
    AA:AC 由薪資表公式自動計算，此處不寫入。
    """
    ...
    qrs = [
        (r[0] if r else "", r[1] if len(r) > 1 else "", r[2] if len(r) > 2 else "")
        for r in qrs
    ]

    vwx = [[q, r, s] for q, r, s in qrs if r and str(r).strip() not in ("", "0")]
    if not vwx:
        _log(log, "    共通流程：R 欄全為 0，跳過")
        return

    ws.batch_clear([f"U2:Y{last_q}"])
    ws.update(f"V2:X{len(vwx) + 1}", vwx, value_input_option="RAW")

    v_col  = [row[0] for row in vwx]
    u_data = [[v_col.count(name)] for name in v_col]
    ws.update(f"U2:U{len(u_data) + 1}", u_data, value_input_option="RAW")

    _log(log, f"    共通流程完成（{len(vwx)} 筆）")
...
def _calc_qrs_direct(
    ...
    ws.update(f"Q{start_row}:Q{end_row}", _get(q_col), value_input_option="RAW")
    ws.update(f"R{start_row}:R{end_row}", _get(r_col), value_input_option="RAW")
    ws.update(f"S{start_row}:S{end_row}", _get(s_col), value_input_option="RAW")
...
def _calc_qrs_salary(
    ...
    ws.update(f"Q{start_row}:Q{end_row}", q_data, value_input_option="RAW")
    ws.update(f"R{start_row}:R{end_row}", r_data, value_input_option="RAW")
    ws.update(f"S{start_row}:S{end_row}", s_data, value_input_option="RAW")
    _log(log, f"    QRS 寫入完成（{num_rows} 列）")
...
def _voucher_expand_qrs(
    ...
    ws.update(f"Q{start_row}:Q{new_end}", q_data, value_input_option="RAW")
    ws.update(f"R{start_row}:R{new_end}", r_data, value_input_option="RAW")
    ws.update(f"S{start_row}:S{new_end}", s_data, value_input_option="RAW")
