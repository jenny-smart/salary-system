...
function api_getOtherContractSettings() {
  if (typeof CentralMaster !== "undefined" && typeof CentralContext !== "undefined") {
    var centralConfig = CentralMaster.getRegionConfig(CentralContext.getRegion());
    return {
      ...
      detectedPeriod: CentralContext.getPeriod()
    };
  }
...
function getConfiguredRootFolderId_() {
  ...
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(OTHER_CONTRACT_PDF_CONFIG.CONFIG_SHEET_NAME);
  if (sheet) {
...
function showToast(msg, title) {
  SpreadsheetApp.getActiveSpreadsheet().toast(msg, title || "執行中", 5);
}
